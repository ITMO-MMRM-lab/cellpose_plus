...
class NucleusInferenceConfig(NucleusConfig):
    """
    Configuration class for inference settings in nucleus detection.

        This class encapsulates configuration parameters used for
        conducting inference with nucleus detection models. It is
        designed to optimize performance and resource management during
        the inference process.

        Attributes:
            GPU_COUNT: The number of GPUs to be used for inference.
            IMAGES_PER_GPU: The number of images to process per GPU.
            IMAGE_RESIZE_MODE: The resizing method to be applied to images.
            RPN_NMS_THRESHOLD: The non-maximum suppression threshold for the
                              region proposal network during inference.
...
class NucleusDataset(utils.Dataset):
    """
    A class for handling and processing the Nucleus dataset used in image segmentation tasks.

    This class provides functionality to load the dataset subsets and generate instance masks for images.
    ...
    Attributes:
        ...
        subset: The specific subset of the dataset being loaded, such as training or validation images.

    The methods allow users to conveniently access the dataset, retrieve masks representing different nuclei instances in an image, and obtain image paths for further processing.
    """
...
def compute_batch_ap(dataset, image_ids, verbose=1):
    """
    Compute the average precision (AP) for a batch of images.

        This method iterates through the specified image IDs, loads each image,
        performs object detection, and calculates the average precision
        at different IoU thresholds ranging from 0.5 to 0.95.

        Args:
            dataset: The dataset containing the images and their annotations.
            image_ids: A list of image identifiers to process.
            verbose: An optional parameter that controls the level of output
                     information printed to the console (default is 1).

        Returns:
            A list of average precision scores for each processed image.
    """
    APs = []
    for image_id in image_ids:
        # Load image
