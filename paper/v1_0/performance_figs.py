...
def suppfig_cellpose_params(test_root, save_root, save_figure=False):
    """
    Generate supplementary figure for Cellpose parameters comparison.

        This method creates a histogram and line plots to compare the average precision
        of Cellpose segmentation results under different conditions. It visualizes the
        differences in performance using histograms, and it plots the average precision
        against the intersection-over-union (IoU) matching thresholds.

        Args:
            test_root: Path to the directory containing test images and masks.
            save_root: Path to the directory where the Cellpose results and figures will be saved.
            save_figure: Flag indicating whether to save the generated figure to disk.

        Returns:
            None: This method does not return a value but saves a figure if specified.
    """
...
def cyto3d(save_root, save_figure=True):
    """
    Generates and optionally saves a 3D performance figure for various model architectures.

        This method evaluates different 3D model architectures by computing the average precision
        for ground truth masks against predicted masks loaded from a specified directory.
        It plots the average precision as a function of the Intersection over Union (IoU) matching
        threshold for each model architecture, and visualizes the results in a figure.

        Args:
            save_root: The root directory where ground truth and predicted mask files are stored.
            save_figure: A boolean to indicate whether to save the generated figure to a file.
                         Defaults to True.

        Returns:
            None: The function does not return any value. It displays or saves the generated plot.
    """
