...
def fig1(imgs_norm, masks_true, dat, timings, save_fig=False):
    """
    Generates a figure for visualizing validation results and performance metrics.

        This method creates a multi-panel figure displaying a validation image,
        true masks, model predictions, test set runtimes, and validation scores
        across different thresholds. It includes visual outlines for true and
        predicted masks and displays F1 scores based on the input data.

        Args:
            imgs_norm: Normalized images to be displayed.
            masks_true: Ground truth masks corresponding to the images.
            dat: A dictionary containing model performance metrics and thresholds.
            timings: Data representing runtime per image for different models.
            save_fig: Flag indicating whether to save the generated figure as a
                PDF file.

        Returns:
            None: This method generates a figure and saves it if requested,
...
def fig2(imgs_norm, masks_true, dat, type_names, types, emb, emb_test, save_fig=False):
    """
    Generate and display a figure comparing model predictions and ground truth.

        This method creates a composite figure that visualizes the prediction outlines from multiple models,
        including ground truth comparisons, as well as t-SNE visualizations of training and test sets.

        Args:
            imgs_norm: Normalized images to be displayed.
            masks_true: Ground truth masks for the images.
            dat: A dictionary containing model predictions and related metrics.
            type_names: Names of the different types of models used in predictions.
            types: Indices indicating the type of each embedding vector.
            emb: Embedding vectors for the training set.
            emb_test: Embedding vectors for the test set.
            save_fig: Boolean flag indicating whether to save the figure as a PDF.

        Returns:
            None: The method does not return a value but saves a figure if requested.
    """
    ids = [0, 3, 56, 55, 81, 75]
