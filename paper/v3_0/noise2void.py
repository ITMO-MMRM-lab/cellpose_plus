...
def train_per_image(img_noisy):
    """
    Trains a noise-to-void (N2V) model on a given noisy image and returns the denoised prediction.

    This method takes a noisy image, divides it into training and validation patches, configures
    a network model, and begins the training process. After training, it predicts and returns
    the denoised version of the noisy image.

    Args:
        img_noisy: A noisy input image from which the model will learn to denoise.

    Returns:
        The denoised prediction of the input noisy image.
    """
    img = img_noisy.copy()
    print(img.shape)
    Ly, Lx = img.shape[-2:]
...
def train_per_image_synthetic(root, ctype="cyto2", plot=False, save=True):
    """
    Train a synthetic image model using noisy test images.

        This method processes a set of noisy images and their corresponding ground truth masks,
        utilizes a segmentation model to evaluate and generate segmentations, and optionally
        plots the results. Additionally, it saves the generated synthetic images and masks if
        specified.

        Args:
            root: The root directory where the noisy test images are stored.
            ctype: The type of Cellpose model to be used for segmentation; defaults to "cyto2".
            plot: A flag indicating whether to plot the images and masks during processing; defaults to False.
            save: A flag indicating whether to save the generated synthetic images and masks; defaults to True.

        Returns:
            A tuple containing:
                - A list of generated synthetic images.
                - A list of masks generated by the segmentation model.
    """
    noise_type = "poisson"
...
def train_test_specialist(root, n_epochs=100, lr=5e-4, test=True):
    """
    Train and test a specialist model for image noise reduction and segmentation.

        This method prepares the training and validation datasets, configures the model,
        trains the model on the noisy images, optionally tests the model on a test set,
        and then saves the predictions and masks. It also computes and returns evaluation
        metrics if testing is enabled.

        Args:
            root: The root directory containing the dataset.
            n_epochs: The number of epochs for training the model.
            lr: The learning rate for the optimizer.
            test: A boolean flag indicating whether to run the testing phase or not.

        Returns:
            Either the minimum validation loss if `test` is False or a tuple containing
            the predicted images, masks, and average precision scores if `test` is True.
    """
    n_train = 3 * 89 * 20
    n_val = 89 * 20
