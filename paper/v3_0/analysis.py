...
def blind_denoising(folder):
    """
    Perform blind denoising on images in a specified folder.

        This method applies two denoising techniques, 'noise2void' and 'noise2self',
        on images categorized as either cytoplasm or nuclei. It loads noisy test data,
        computes average precision for the resulting masks, and prints the results.

        Args:
            folder: The path to the folder containing the images for denoising.
                    It is expected to contain subdirectories for image types.

        Returns:
            None: This method does not return any value. It prints the average precision
...
def real_examples_ribo(root):
    """
    Process and evaluate denoising of ribosomal images.

        This method reads a series of noisy ribosomal images, computes their
        clean counterparts, and applies various denoising techniques to evaluate
        their performance. The results, including average precision scores and
        masks from both noisy and denoised images, are saved to specified files.

        Args:
            root: The directory path where the input image files are located
                  and where the output data files will be saved.

        Returns:
            None: This method does not return a value. It saves the processed
            data to .npy files in the specified directory.
    """
