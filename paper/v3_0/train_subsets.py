...
def main():
    """
    Run the main training process for the Denoise model.

        This method sets up a command-line interface to handle parameters for training
        a Denoise model using images and labels from the specified dataset. It prepares
        the training and test data, initializes the model, and performs the training
        process.
...
def save_results(folder, sroot):
    """
    Save segmentation performance results for different model configurations.

        This method evaluates a Cellpose segmentation model with varying subsampling
        and training epochs on a set of noisy test images. It computes the average precision
        at a threshold of 0.5 and saves the results to NumPy files for further analysis.

        Args:
            folder: The directory path where input images and results will be stored.
            sroot: The root path where pre-trained model weights are located.
