...
def logger_setup(
    ...
    Set up logging configuration for the application.
        ...
        This method creates a logging setup by establishing a log directory, generating a log file, and
        configuring the logging handlers. It directs log output either to a specified log file or to
        standard output, depending on the user's preference.

        Args:
            cp_path: The relative path for the logging directory.
            logfile_name: The name of the log file to create or overwrite.
            stdout_file_replacement: An optional file path to replace standard output with a file.
                                     If None, standard output will be used.
...
def check_dir(path):
    ...
    Check if a directory exists and create it if it does not.
        ...
        This method checks if the specified directory exists. If the directory
        does not exist, it creates the directory.

        Args:
            path: The path of the directory to check and potentially create.
...
def outlines_to_text(base, outlines):
    """
    Converts a list of outlines into a text file.

        This method flattens each outline in the provided list and writes the
        coordinates of each outline into a text file named with the base name
        followed by "_cp_outlines.txt". Each set of coordinates is written on
        a new line, with values separated by commas.

        Args:
            base: The base name for the output text file.
            outlines: A list of outline objects that can be flattened to obtain
...
def load_dax(filename):
    """
    Load a DAX file and extract image data from it.

        This method reads a DAX file and loads its corresponding metadata from an
        associated INF file. It extracts the image dimensions, the number of frames,
        and determines the endianness. The method then reads the DAX file into a
        memory-mapped array and returns the image data.

        Args:
            filename: The name of the DAX file to be loaded.

        Returns:
            A NumPy array containing the image data if successful;
            None if the INF file is not found.
    """
